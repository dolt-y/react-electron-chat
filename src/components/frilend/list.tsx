...
export const FriendList: React.FC = () => {
  ...
  let listContent = <div className={styles.state}>暂无好友</div>;
  if (loading) {
    listContent = <div className={styles.state}>加载中...</div>;
  } else if (error) {
    listContent = <div className={styles.state + " " + styles.error}>{error}</div>;
    ...
    listContent = (
      <ul className={styles.list}>
        {friends.map((friend, index) => (
          <li
            key={friend.id}
            className={[styles.item, selectedFriend?.id === friend.id && styles.active].filter(Boolean).join(" ")}
  ...
  return (
    <ResizableSidebar>
      <div className={styles.container}>
        ...
        <div className={styles.detailPanel}>{detailContent}</div>
      </div>
    </ResizableSidebar>
  );
};
